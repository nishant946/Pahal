...
export const registerTeacher = async (req, res) => {
  try {
    ...
    const preferredDaysArray = Array.isArray(preferredDays)
      ? preferredDays
      : [];
    const subjectChoicesArray = Array.isArray(subjectChoices)
      ? subjectChoices
      : [];
    ...
    if (preferredDaysArray.length === 0) {
      return res
        .status(400)
        .json({ message: "At least one preferred day must be provided" });
    }

    if (subjectChoicesArray.length === 0) {
      return res
        .status(400)
        .json({ message: "At least one subject choice must be provided" });
    }

    const formattedPreferredDay = preferredDaysArray.map(
    ...
    const teacher = new Teacher({
      ...
      email,
      department,
      preferredDays: formattedPreferredDay,
...
export const loginTeacher = async (req, res) => {
  try {
    ...
    if (!teacher.isActive) {
      return res
        .status(401)
        .json({ message: "Teacher account is deactivated" });
    }
...
export const getTeacherStats = async (req, res) => {
  try {
    ...
    const unverifiedTeachers = await Teacher.countDocuments({
      isActive: true,
      isVerified: false,
    });
