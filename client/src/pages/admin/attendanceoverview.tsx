...
const AttendanceOverview: React.FC = () => {
  ...
  const [studentsOnDate, setStudentsOnDate] = useState<StudentsOnDate | null>(
    null
  );

  // Teacher attendance state
  const [teacherOverview, setTeacherOverview] =
    useState<TeacherAttendanceOverview>({
      todayAttendance: 0,
      totalTeachers: 0,
      todayAttendancePercentage: 0,
      weeklyAttendance: [],
  ...
  const [teacherAnalytics, setTeacherAnalytics] =
    useState<TeacherAttendanceAnalytics | null>(null);
  const [teachersOnDate, setTeachersOnDate] = useState<TeachersOnDate | null>(
    null
  );
    ...
    try {
      ...
      setError(
        err.response?.data?.message ||
          "Failed to fetch teacher attendance overview"
      );
    } finally {
      setLoading(false);
    }
  ...
  const fetchTeacherAttendanceAnalytics = async () => {
    try {
      ...
      const response = await api.get(
        `/admin/teacher-attendance/analytics?${params}`
      );
      setTeacherAnalytics(response.data);
    } catch (err: any) {
      setError(
        err.response?.data?.message ||
          "Failed to fetch teacher attendance analytics"
      );
    } finally {
      setAnalyticsLoading(false);
    }
  ...
  const fetchStudentsOnDate = async () => {
    try {
      setAnalyticsLoading(true);
      const response = await api.get(
        `/admin/attendance/students?date=${selectedDate}`
      );
  ...
  const fetchTeachersOnDate = async () => {
    try {
      setAnalyticsLoading(true);
      const response = await api.get(
        `/admin/teacher-attendance/teachers?date=${selectedDate}`
      );
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Monitor and analyze attendance across the institution
          </p>
        </div>
      ...
      {activeTab === "overview" && (
        <div className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-4">
              ...
              <CardContent>
                <div className="text-2xl font-bold text-green-600">
                  {attendanceType === "students"
                    ...
                    : teacherOverview.todayAttendance}
                </div>
                <p className="text-xs text-gray-600">
                  {(attendanceType === "students"
                    ? overview.todayAttendancePercentage
                    : teacherOverview.todayAttendancePercentage
                  ).toFixed(1)}
                  % attendance rate
                </p>
              </CardContent>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-red-600">
                  {attendanceType === "students"
                    ...
                    : teacherOverview.absentToday}
                </div>
                <p className="text-xs text-gray-600">
                  {attendanceType === "students"
                    ? (
                        (overview.absentToday / overview.totalStudents) *
                        100
                      ).toFixed(1)
                    : (
                        (teacherOverview.absentToday /
                          teacherOverview.totalTeachers) *
                        100
                      ).toFixed(1)}
                  % absent rate
                </p>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">
                  Total{" "}
                  {attendanceType === "students" ? "Students" : "Teachers"}
                </CardTitle>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                ...
                {(attendanceType === "students"
                  ? overview.monthlyAttendancePercentage
                  : teacherOverview.monthlyAttendancePercentage) >= 80 ? (
                ...
                <div
                  className={`text-2xl font-bold ${
                    (attendanceType === "students"
                      ...
                      : (attendanceType === "students"
                          ? overview.monthlyAttendancePercentage
                          : teacherOverview.monthlyAttendancePercentage) >= 60
                      ? "text-yellow-600"
                      : "text-red-600"
                  }`}
                >
                  {(attendanceType === "students"
                    ? overview.monthlyAttendancePercentage
                    : teacherOverview.monthlyAttendancePercentage
                  ).toFixed(1)}
                  %
                </div>
                <p className="text-xs text-gray-600">
          ...
          <Card>
            ...
            <CardContent>
              {(attendanceType === "students"
                ? overview.gradeWiseStats
                : teacherOverview.departmentWiseStats
              ).length > 0 ? (
          ...
          <Card>
            ...
            <CardContent>
              {(attendanceType === "students"
                ...
                : teacherOverview.weeklyAttendance
              ).length > 0 ? (
                ...
                <CardContent>
                  {(attendanceType === "students"
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {attendanceType === "students"
                          ? analytics!.lowAttendanceStudents.map(
                              (student, index) => (
                                <TableRow key={index}>
                          ...
                          : teacherAnalytics!.lowAttendanceTeachers.map(
                              (teacher, index) => (
                                <TableRow key={index}>
                                  <TableCell className="font-medium">
                                    {teacher.name}
                                  ...
                                  <TableCell>
                                    <span
                                      className={`font-bold ${
                                        teacher.attendancePercentage < 60
                                          ? "text-red-600"
                                          : "text-yellow-600"
                                      }`}
                                    >
                    ...
                    <div className="text-center py-8 text-gray-500">
                      No{" "}
                      {attendanceType === "students" ? "students" : "teachers"}{" "}
                      with low attendance in the selected period
                    </div>
                  )}
                </CardContent>
              </Card>
                        ...
                        <div key={day._id} className="space-y-2">
                          ...
                          <div className="flex h-4 bg-gray-200 rounded">
                            ...
                            <div
                              ...
                              style={{
                                width: `${
                                  (day.absent / (day.present + day.absent)) *
                                  100
                                }%`,
                              }}
                            ></div>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold text-green-600">
                      {attendanceType === "students"
                        ...
                        : teachersOnDate!.teachers.filter(
                            (t) => t.status === "present"
                          ).length}
                    </div>
                  </CardContent>
                      ...
                      {attendanceType === "students"
                        ...
                        ? (
                            (teachersOnDate!.teachers.filter(
                              ...
                              teachersOnDate!.teachers.length) *
                            100
                          ).toFixed(1)
                        : 0}
                      %
