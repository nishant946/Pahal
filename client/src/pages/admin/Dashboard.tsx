...
const AdminDashboard: React.FC = () => {
  ...
  const fetchAllData = async () => {
    try {
      ...
      const attendanceData = attendanceResponse.data as AttendanceStats;
      ...
      setQuickActions({
        pendingVerifications: dashboardData.unverifiedTeachers,
        lowAttendanceAlerts:
          attendanceData.todayAttendancePercentage < 75 ? 1 : 0,
        recentRegistrations: dashboardData.recentActivities.filter(
          (activity) => activity.action === "Teacher Registration"
        ).length,
      });
  ...
  const attendanceStatus = getAttendanceStatus(
    attendanceStats.todayAttendancePercentage
  );
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {getCurrentGreeting()}, {teacher?.name}! Here's your institution
            overview.
          </p>
        </div>
                ...
                style={{
                  width: `${
                    stats.totalTeachers > 0
                      ? (stats.verifiedTeachers / stats.totalTeachers) * 100
                      : 0
                  }%`,
                }}
              ></div>
            ...
            <div className="w-full bg-muted rounded-full h-1.5 mt-2">
              <div
                className={`h-1.5 rounded-full transition-all duration-300 ${
                  attendanceStats.todayAttendancePercentage >= 75
                    ? "bg-green-500 dark:bg-green-400"
                    : attendanceStats.todayAttendancePercentage >= 60
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              <span className="text-red-600 font-medium">
                {attendanceStats.absentToday} absent today
              </span>
            </p>
            ...
            <div
              className={`text-2xl font-bold ${
                attendanceStats.monthlyAttendancePercentage >= 80
                  ...
                  : attendanceStats.monthlyAttendancePercentage >= 60
        ...
        <Card className="lg:col-span-1">
          ...
          <CardContent className="space-y-3">
            {quickActions.pendingVerifications > 0 && (
              <div className="flex items-center p-3 bg-yellow-50 dark:bg-yellow-950/20 rounded-lg border border-yellow-200 dark:border-yellow-800">
                <Clock className="h-4 w-4 text-yellow-600 dark:text-yellow-400 mr-2" />
                <div className="flex-1">
                  <p className="text-sm font-medium text-yellow-800 dark:text-yellow-200">
                    {quickActions.pendingVerifications} teacher
                    {quickActions.pendingVerifications > 1 ? "s" : ""} awaiting
                    verification
                  </p>
                </div>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <div className="text-2xl font-bold text-green-600">
                {Math.round(
                  (stats.verifiedTeachers / Math.max(stats.totalTeachers, 1)) *
                    100
                )}
                %
              </div>
