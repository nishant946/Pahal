...
import { RefreshCw, Search, SortAsc, SortDesc, Filter, Users, UserCheck, UserX, UserPlus, Edit3 } from "lucide-react";
...
const ProgressPage: React.FC = () => {
  ...
  const [sortBy, setSortBy] = useState<"name" | "rollNumber" | "mentor" | "lastUpdate">("name");
  const [sortOrder, setSortOrder] = useState<"asc" | "desc">("asc");
  const [filterBy, setFilterBy] = useState<"all" | "with-mentor" | "without-mentor" | "with-progress" | "without-progress">("all");
  ...
  const [selectedMentorForStudent, setSelectedMentorForStudent] = useState<Record<string, string>>({});
  ...
  const filteredAndSortedStudents = useMemo(() => {
    let filtered = students.filter(student => {
      ...
      const matchesSearch = student.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                           ...
                           (student.mentor && student.mentor.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  useEffect(() => {
    const fetchStudentsAndProgress = async () => {
      try {
        ...
        else if (Array.isArray(studentsData.students)) studentsList = studentsData.students;
        
        // Sort students alphabetically by name by default
        studentsList.sort((a: Student, b: Student) => a.name.localeCompare(b.name));
        
        // Filter active and verified teachers
        const activeTeachersList = teachersData.filter((teacher: Teacher) => 
  ...
  const handleChangeMentor = async (studentId: string, newMentorName: string) => {
    ...
    <Layout>
      <div className="p-3 sm:p-4 md:p-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold">Progress Management</h1>
            <p className="text-sm text-gray-600 mt-1">
              Total: {students.length} | Displayed: {filteredAndSortedStudents.length}
            </p>
          </div>
          <Button 
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm text-gray-600">Without Mentor</p>
                  <p className="text-2xl font-bold">
                    {students.filter(s => !s.mentor || s.mentor === "").length}
                  </p>
                </div>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm text-gray-600">With Progress</p>
                  <p className="text-2xl font-bold">
                    {students.filter(s => latestProgressMap[s.id]?.progress).length}
                  </p>
                </div>
              </div>
            </CardContent>
        ...
        {/* Students Grid */}
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 xl:grid-cols-3">
          {Array.isArray(filteredAndSortedStudents) && filteredAndSortedStudents.length > 0 ? (
            filteredAndSortedStudents.map((student) => (
              <div key={student.id} className="bg-white rounded-lg shadow hover:shadow-lg transition-all duration-200 p-4 flex flex-col gap-2 transform hover:scale-[1.02]">
                ...
                <div className="text-xs text-gray-400 mb-2">
                  ...
                  {latestProgressMap[student.id]?.timestamp && (
                    <span className="block">
                      {new Date(latestProgressMap[student.id]!.timestamp).toLocaleDateString()}
                    </span>
                  )}
                </div>
                <Button 
